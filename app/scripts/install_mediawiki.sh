...
if [ -d "$MEDIAWIKI_DIR" ]; then
  ...
  if [ "$MEDIAWIKI_OVERRIDE" == true ]; then
    ...
    echo "MEDIAWIKI_DIR exist already. Stop here... Override is disabled. Enable with: export MEDIAWIKI_OVERRIDE=true"
    exit 0
  fi
fi
...
curl -XPUT http://$ES_HOST:9200/_all/_settings?preserve_existing=true -d '{"index.mapping.total_fields.limit" : "2000"}'
...
echo "Setup complete. Run /scripts/get_data.sh to load demo data."
